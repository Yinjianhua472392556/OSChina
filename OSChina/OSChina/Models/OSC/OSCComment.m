...
+ (NSAttributedString *)attributedTextFromReplies:(NSArray *)replies {
    ...
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"\n\n--共有%lu条评论--\n",replies.count] attributes:@{NSForegroundColorAttributeName : [UIColor grayColor]}];
    
    [replies enumerateObjectsUsingBlock:^(OSCReply *reply, NSUInteger idx, BOOL * _Nonnull stop) {
        
        NSMutableAttributedString *commentString = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@: ",reply.author] attributes:@{NSForegroundColorAttributeName : [UIColor nameColor], NSFontAttributeName : [UIFont systemFontOfSize:13]}];
        ...
        [replyContent addAttributes:@{NSForegroundColorAttributeName : [UIColor grayColor], NSFontAttributeName : [UIFont systemFontOfSize:13]} range:NSMakeRange(0, replyContent.length)];
