...
- (instancetype)initWithCommentType:(CommentType)commentType andObjectID:(int64_t)objectID {
    ...
    if (self) {
        ...
        self.generateURL = ^NSString *(NSUInteger page) {
            ...
            NSString *URL;
            if (commentType <= 4) {
                URL = [NSString stringWithFormat:@"%@%@?catalog=%lu&id=%lld&pageIndex=%lu&%@",OSCAPI_PREFIX,OSCAPI_COMMENTS_LIST,(unsigned long)commentType,objectID,page,OSCAPI_SUFFIX];
            }else if (commentType == CommentTypeBlog) {
            
                URL = [NSString stringWithFormat:@"%@%@?id=%lld&pageIndex=%lu&%@",OSCAPI_PREFIX,OSCAPI_BLOGCOMMENTS_LIST,objectID,page,OSCAPI_SUFFIX];
            }else if (commentType == CommentTypeSoftware) {
            
                URL = [NSString stringWithFormat:@"%@%@?project=%lld&pageIndex=%lu&%@",OSCAPI_PREFIX,OSCAPI_SOFTWARE_TWEET_LIST,objectID,page,OSCAPI_SUFFIX];
            }
            return URL;
        };
...
- (void)setBlockForCommentCell:(CommentCell *)cell {
    ...
    cell.deleteObject = ^ (UITableViewCell *cell) {
        ...
        [manager POST:[NSString stringWithFormat:@"%@%@?",OSCAPI_PREFIX,OSCAPI_COMMENT_DELETE] parameters:@{@"catalog" : @(_commentType), @"id" : @(_objectID), @"replyid" : @(comment.commentID), @"authorid" : @(comment.authorID)} success:^(AFHTTPRequestOperation * _Nonnull operation, ONOXMLDocument *responseObject) {
