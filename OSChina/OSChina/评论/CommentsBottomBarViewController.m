...
- (void)setUpBlock {
    ...
    _commentsVC.didCommentSelected = ^(OSCComment *comment) {
        ...
        if (weakSelf.replyID == comment.commentID) {
            ...
            weakSelf.replyUID = comment.commentID;
...
- (void)sendContent {
    ...
    NSDictionary *parameters = _commentType == CommentTypeBlog? @{@"blog" : @(_objectID), @"uid" : @([Config getOwnID]), @"reply_id" : @(_replyID), @"objuid" : @(_replyUID), @"content" : [Utils convertRichTextToRawText:self.editingBar.editView]}  :  @{@"catalog": @(_commentType), @"id": @(_objectID),@"uid": @([Config getOwnID]),@"replyid": @(_replyID),@"authorid": @(_replyUID),@"content": [Utils convertRichTextToRawText:self.editingBar.editView],@"isPostToMyZone": @(0)};
