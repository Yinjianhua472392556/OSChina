...
@property (nonatomic, strong) NSMutableArray *opitionButtons;
...
- (void)dawnAndNightMode:(NSNotification *)center {
    ...
    [[UITabBar appearance] setBarTintColor:[UIColor titleColor]];
    ...
    [self.viewControllers enumerateObjectsUsingBlock:^( UINavigationController *nav, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx == 0) {
            ...
            [newsVc.viewPager.controllers enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [tweetVc.viewPager.controllers enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                UITableViewController *table = obj;
                [table.navigationController.navigationBar setBarTintColor:[UIColor navigationbarColor]];
                [table.tabBarController.tabBar setBarTintColor:[UIColor titleBarColor]];
                [table.tableView reloadData];
...
- (void)viewDidLoad {
    ...
    blogViewCtl = [[BlogsViewController alloc] initWithBlogsType:BlogsTypeLatest];
    recommendBlogViewCtl = [[BlogsViewController alloc] initWithBlogsType:BlogsTypeRecommended];
    ...
    SwipableViewController *newsSVC = [[SwipableViewController alloc] initWithTitle:@"综合" andSubTitles:@[@"资讯", @"热点", @"博客", @"推荐"] andControllers:@[newsViewCtl,hotNewsViewCtl,blogViewCtl,recommendBlogViewCtl] underTabbar:YES];
    
    SwipableViewController *tweetsSVC = [[SwipableViewController alloc] initWithTitle:@"动弹" andSubTitles:@[@"最新动弹", @"热门动弹", @"我的动弹"] andControllers:@[newTweetViewCtl,hotTweetViewCtl,myTweetViewCtl] underTabbar:YES];
    ...
    self.viewControllers = @[[self addNavigationItemForViewController:newsSVC],[self addNavigationItemForViewController:tweetsSVC],[UIViewController new],discoverNav,homepageNav];
    NSArray *titles = @[@"综合", @"动弹", @"", @"发现", @"我"];
    NSArray *images = @[@"tabbar-news", @"tabbar-tweet", @"", @"tabbar-discover", @"tabbar-me"];
    [self.tabBar.items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj setTitle:titles[idx]];
        [obj setImage:[UIImage imageNamed:images[idx]]];
        [obj setSelectedImage:[UIImage imageNamed:[images[idx] stringByAppendingString:@"-selected"]]];
    }];
    ...
    // 功能键相关
    _opitionButtons = [NSMutableArray new];
    ...
    for (int i = 0; i < 6; i++) {
        OptionButton *optionButton = [[OptionButton alloc] initWithTitle:buttonTitles[i] image:[UIImage imageNamed:buttonImages[i]] andColor:[UIColor colorWithHex:buttonColors[i]]];
        optionButton.frame = CGRectMake(_screenWidth/6 * (i%3*2+1) - (_length+16)/2,  _screenHeight + 150 + i/3*100, _length + 16, _length + [UIFont systemFontOfSize:14].lineHeight + 24);
...
- (UINavigationController *)addNavigationItemForViewController:(UIViewController *)viewController {
    ...
    viewController.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action:@selector(pushSearchViewController)];
...
- (void)changeTheButtonStateAnimatedToOpen:(BOOL)isPressed {
    ...
    if (isPressed) {
        ...
        for (int i = 0; i < 6; i++) {
            UIButton *button = _opitionButtons[i];
            UIAttachmentBehavior *attachment = [[UIAttachmentBehavior alloc] initWithItem:button attachedToAnchor:CGPointMake(_screenWidth/6 * (i%3*2+1), _screenHeight + 200 + i/3*100)];
...
- (void)pushSearchViewController {
    [(UINavigationController *)self.selectedViewController pushViewController:[UIViewController new] animated:YES];
}
