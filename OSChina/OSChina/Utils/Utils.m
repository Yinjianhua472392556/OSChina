...
+ (NSString *)HTMLWithData:(NSDictionary *)data usingTemplate:(NSString *)templateName
{
    ...
    [mutableData setObject:@(((AppDelegate *)[UIApplication sharedApplication].delegate).inNightMode)
                    forKey:@"night"];
    NSString *temp = [GRMustacheTemplate renderObject:mutableData fromString:template error:nil];
...
+ (NSString *)convertRichTextToRawText:(UITextView *)textView {
    ...
    [textView.attributedText enumerateAttribute:NSAttachmentAttributeName inRange:NSMakeRange(0, textView.attributedText.length) options:NSAttributedStringEnumerationReverse usingBlock:^(NSTextAttachment *attachment, NSRange range, BOOL * _Nonnull stop) {
        if (!attachment) {
            return ;
        }
